...
module.exports = new (function() {
    ...
    this.handleMethod = function(matchId, elementId, method, user, methodData) {
        ...
        if (user.id === 0) throw new MethodError('match.notLoggedIn');
    ...
    this.registerFrontendEvents = function(eventEmitter) {
         ...
         * {"data": true}
         */
        eventEmitter.on('match.method', function(user, data, callback, errorCallback) {
            try {
                self.handleMethod(data.matchId, data.elementId, data.method, user, data.data);
                callback(true);
            } catch (e) {
