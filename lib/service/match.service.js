...
module.exports = new (function() {
    ...
    this.findByGameSocket = function(socket) {
        for (let i in this.matches) {
            ...
            if (this.matches[i].gameSocket === socket) {
    ...
    this.registerGameEvents = function(eventEmitter) {
        eventEmitter.on('connectionOpened', function(socket) {
            self.sendCommand(socket, 'match.auth');
        });

        eventEmitter.on('match.auth', function(socket, data) {
            let match = self.findById(data.id);

            if (match === null) {
                self.sendCommand(socket, 'match.authError', 'invalidId');
            }
            if (match.socket !== null) {
                self.sendCommand(socket, 'match.authError', 'alreadyConnected');
            }

            match.socket = socket;

            match.sendCommand('match.start');
        });

        eventEmitter.on('event', function(socket, data) {
            let match = self.findByGameSocket(socket);

            if (match === null) {
                self.sendCommand(socket, 'match.eventError', 'matchNotFound');
            }

            match.handleEvent(data.event, data);
        });

        eventEmitter.on('connectionClosed', function(socket) {
            let match = self.findByGameSocket(socket);
            if (match !== null) {
                self.cancelMatch(match);
            }
        });
    };
    ...
    this.registerFrontendEvents = function(eventEmitter) {
        ...
        eventEmitter.on('match.getMatch', function(user, data, callback, errorCallback) {
            try {
                callback(self.findById(data.matchId));
            } catch (e) {
                if (e instanceof EventError) {
                    errorCallback(e.key);
                } else {
