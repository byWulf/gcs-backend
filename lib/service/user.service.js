...
module.exports = new (function() {
    ...
    this.hasRole = function(socketId, role) {
        let user = this.findBySocketId(socketId);

        return user !== null && (user.roles || []).indexOf(role) > -1;
    };

    this.setCurrentMatch = function(userId, match) {
        let currentMatch = match ? {id: match.id, gameName: match.game.name} : null;

        for (let socketId in sockets) {
            if (!sockets.hasOwnProperty(socketId)) continue;
            if (sockets[socketId]._id.toString() !== userId) continue;

            sockets[socketId].currentMatch = currentMatch;
        }

        databaseCommunicator.update('users', {_id: userId}, {$set: {currentMatch: currentMatch}});

        this.sendUpdateToSubscribers(userId, 'user.currentMatchChanged', {currentMatch: currentMatch});
    };

    this.sendUpdateToSubscribers = function(userId, updateKey, updateData) {
        for (let socketId in sockets) {
            if (!sockets.hasOwnProperty(socketId)) continue;
            if (sockets[socketId]._id.toString() !== userId) continue;

            this.frontendEmitter.emit('sendToSocket', socketId, updateKey, {id: userId, data: updateData});
        }
    };
    ...
    this.registerEvents = function(eventEmitter) {
         ...
         *
         * Response:
         * {"data": {"id": "asdf", "displayName": "Wulf", "roles": ["user"], "authToken": authToken, "currentMatch": null}}
         */
         ...
         *
         * Response:
         * {"data": {"id": "asdf", "displayName": "Wulf", "roles": ["user"], "authToken": authToke, "currentMatch": {"id": "1234asdf", "gameName": "Foobar"}}}
         */
         ...
         *
         * Response:
         * {"data": {"id": "asdf", "displayName": "Wulf", "roles": ["user"], "authToken": authToken, "currentMatch": {"id": "1234asdf", "gameName": "Foobar"}}}
         */
